...
PERFORMANCE_CURVE_VERSION = "2026-08-27-v1-pit-model-health"
...
def _cohort_return(group: pd.DataFrame, horizon: int) -> float:
    """Equal-weight one cross-section per signal date; percent return."""
    values = _numeric(group, f"Return{horizon}D").dropna()
    if values.empty:
        return float("nan")
...
def build_performance_curve(history: pd.DataFrame) -> pd.DataFrame:
    ...
    data = history.copy()
    data["Date"] = pd.to_datetime(data["TradeDate"], errors="coerce").dt.normalize()
