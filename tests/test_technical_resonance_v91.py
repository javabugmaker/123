"""Behavioural coverage for :mod:`technical_resonance_v90`.

The five-factor resonance overlay ships as a diagnostic: it never changes a
score, it only annotates backtest samples so the research page can show
whether resonance bands separate outcomes.  That makes it easy to break
silently -- a wrong factor would still produce a plausible-looking table --
so the semantics are pinned here instead.

Recovered from ``experiment/five-factor-resonance-v90`` (commit ``af66bf8``)
before that branch was deleted: the branch's v90 implementation was later
superseded by the vectorised v91 module now on ``main``, but its tests were
never migrated and still describe the intended behaviour.
"""
