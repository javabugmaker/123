...
def _candidate_row(row: dict[str, str], rank: int) -> str:
...
<td class="number">{_safe(_fmt(row.get("Close"), 2))}</td><td class="number score">{_safe(_fmt(alpha))}</td>
...
def _table(rows: list[dict[str, str]]) -> str:
    body = "".join(_candidate_row(row, index) for index, row in enumerate(rows, 1))
    return f'''<div class="table-wrap"><table><thead><tr><th>#</th><th>代码 / 名称</th><th>类型</th><th>行业 / 主题</th><th>收盘</th><th>ALPHA</th><th>状态</th><th>信号</th><th>参考买点</th><th>止损</th><th>目标</th><th>当前解释</th></tr></thead>
...
<title>{_safe(report_date)} · A股研究简报</title><link rel="stylesheet" href="{_safe(asset_prefix)}assets/report-v114.css"></head><body>
...
<section class="section"><div class="section-head"><div><span>02 / RESEARCH UNIVERSE</span><h2>研究候选池</h2></div><p id="visible-count">{len(table_rows)} ROWS</p></div><div class="filters"><input id="candidate-search" type="search" placeholder="搜索代码、名称、行业"><select id="candidate-state"><option value="">全部状态</option><option>READY</option><option>CAUTIOUS</option><option>OBSERVE</option><option>BLOCKED</option></select><select id="candidate-asset"><option value="">全部类型</option><option>股票</option><option>ETF</option></select></div>{_table(table_rows)}</section>
...
<script src="{_safe(asset_prefix)}assets/report-v114.js" defer></script></body></html>'''
...
def build_web_report(
    ...
    _atomic_write(site_dir / "assets" / "report-v114.css", PUBLICATION_CSS)
    _atomic_write(site_dir / "assets" / "report-v114.js", PUBLICATION_JS)
