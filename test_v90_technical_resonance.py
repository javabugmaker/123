...
class TechnicalResonanceV90Tests(unittest.TestCase):
    def test_strong_uptrend_reaches_four_or_five_votes(self) -> None:
        frame = _trend_frame(1.0)
        resonance = compute_five_factor_resonance(frame)
        last = resonance.iloc[-1]
        self.assertEqual(last["ResonanceAvailable"], 5)
        self.assertLessEqual(4, last["ResonanceCount"])
        self.assertLessEqual(last["ResonanceCount"], 5)
        self.assertTrue(bool(last["ResonanceStrongBull"]))
        self.assertTrue(bool(last["ResonanceMACDBull"]))
        self.assertTrue(bool(last["ResonanceRSIBull"]))
        self.assertTrue(bool(last["ResonanceOBVBull"]))
        self.assertTrue(bool(last["ResonanceBOLLBull"]))

    def test_strong_downtrend_stays_low_resonance(self) -> None:
        frame = _trend_frame(-1.0)
        resonance = compute_five_factor_resonance(frame)
        last = resonance.iloc[-1]
        self.assertEqual(last["ResonanceAvailable"], 5)
        self.assertLessEqual(last["ResonanceCount"], 1)
        self.assertTrue(bool(last["ResonanceStrongBear"]))

    def test_sample_attachment_uses_signal_date_not_entry_date(self) -> None:
        ...
        for column in ("Close", "High", "Low", "Open"):
            frame.iloc[
                signal_position + 1 :, frame.columns.get_loc(column)
            ] += 12.0

        resonance = compute_five_factor_resonance(frame)
        expected = resonance.iloc[signal_position]
        samples = [
            {
                "ticker": "000001.SZ",
        ...
        self.assertEqual(attached[0]["resonance_version"], RESONANCE_VERSION)
        self.assertEqual(
            attached[0]["resonance_count"], expected["ResonanceCount"]
        )
        self.assertNotEqual(
            attached[0]["resonance_count"],
            resonance.iloc[signal_position + 1]["ResonanceCount"],
        )

    def test_summary_keeps_resonance_as_diagnostic_groups(self) -> None:
        frame = pd.DataFrame(
            {
        ...
        self.assertEqual(analysis["status"], "EXPERIMENTAL_DIAGNOSTIC_ONLY")
        bands = {row["group"]: row for row in analysis["by_band"]}
        self.assertEqual(bands["4-5/5"]["samples"], 2)
        self.assertEqual(bands["4-5/5"]["average_net_excess_20d"], 6.0)
        transitions = {row["group"]: row for row in analysis["by_transition"]}
        self.assertEqual(transitions["RISING_TO_4PLUS"]["samples"], 1)
