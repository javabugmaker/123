...
## 26. 预算覆盖：把「手写名单」换成「机械规则」，并消灭幽灵余量

提交 `4f927be`。两个新闸门，回答两个此前没人问的问题。

### 26.1 实测推翻了「按大小挑目标」

原计划是给 5 个最大的未覆盖模块补预算。先量了近 30 次提交的字节增量，
结论完全相反：

| 文件 | 磁盘 | 近 30 提交增量 | 提交数 | 原计划 |
|---|---|---|---|---|
| `fundamental_quality.py` | 22,835 | **+6,481** | 12 | 不在名单里 |
| `gui_core.py` | 104,866 | +2,869 | 17 | 已有预算（只剩 134 B） |
| `web_report_v84.py` | 46,307 | +1,588 | 3 | 不在名单里（overlay） |
| `gui.py` | 94,033 | +389 | 60 | 已有预算 |
| `daily_pipeline_core.py` | 42,270 | +120 | 3 | 在名单里 |
| `score_core.py` | 44,289 | **0** | 4 | 在名单里 |
| `downloader_core.py` | 30,543 | **0** | 4 | 在名单里 |
| `model_calibration.py` | 32,222 | **0** | 10 | 不在名单里 |
| `main_core.py` | 22,423 | **-647** | 4 | 在名单里 |
| `filters_core.py` | 21,967 | **0** | 1 | 在名单里 |

**按大小挑的 5 个里，4 个一个字节都没变**——它们是静态债务，不是活跃增长。
真正长的 `fundamental_quality.py` 不在任何人的大文件名单里，也没有任何预算。

教训同 25.3：**锚点选错，闸门就只是在保护一个错误的集合。** 这里是"名单按
大小而非按增长"。改成机械规则后，新文件落地即被拦下，不依赖有人记得更新名单。

### 26.2 两个闸门

| 闸门 | 问的问题 | 阈值 |
|---|---|---|
| `test_every_large_root_module_has_a_budget` | 有没有东西在没人看着的地方长？ | 20,000 B |
| `test_size_budgets_are_not_vacuous` | 已有的预算里，有没有根本不可能触发的？ | 余量 ≤ 4,096 B |

第二个闸门起因是 `scanner.py`：**T1 把它从 72,303 字节掏成 941 字节的 facade，
预算还留在 80,000** —— 79 KB 幽灵余量，文件要涨 85 倍才会报警。它看起来是个
闸门，实际不是。与 `analytics_core` 那 17,417 字节同类，严重 4.5 倍。

### 26.3 结果

| | 之前 | 现在 |
|---|---|---|
| 有预算的根级模块 | 6 / 23 超阈值 | **24（含低于阈值的 `scanner.py`）** |
| 超阈值未覆盖 | 17 | **0** |
| 预算余量合计 | 109,228 B | **31,684 B**（回收 77,544） |
| `scanner.py` 预算 | 80,000（文件 941 B） | **2,048** |
| `analytics_core.py` 预算 | 160,000 | **145,000**（锁掉 T2′ 遗留） |
| `gui.py` 预算 | 100,000 | **98,000**（余量 5,967 超限） |

7 个版本化 overlay 一并纳入预算。理由：**"冻结的补丁"不等于"不会被改"**——
`web_report_v84` 近 3 次提交长了 1,588 字节。不为它们开口子，规则才不用争论。

### 26.4 反向验证

* 新建 21,009 字节的无预算根级文件 → 覆盖闸门报出该文件；
* 把 `scanner.py` 预算改回 80,000 → 幽灵余量闸门报 `80000 vs 941`。

### 26.5 新的窒息点

`gui_core.py` 剩 **134 字节**（104,866 / 105,000），且近 30 提交长了 2,869 字节。
按这个速度几周内就会撞墙，而它能用的办法已经不多了（recon 的类方法盲区已修好，
现在能真正评估它的 1,921 行 `ScannerGUI` 类）。

### 26.6 新增的可复用做法

19. **覆盖类闸门的名单必须机械生成，不能手写** —— 手写名单保护的是"有人记得
...
## 28. 字节预算与换行符不兼容：新克隆假红，以及一个"假窒息点"

干净树验证（`git clone` 到临时目录后跑全量）抓到
`test_legacy_giant_modules_are_shrink_only` 报
`{'analytics_core.py': 146065}`，超预算 1,065 字节。

### 28.1 根因

`core.autocrlf=true`。索引里 `analytics_core.py` 是 LF（142,583 B），
检出后变 CRLF（146,065 B，**每行多 1 字节**）。而 `4f927be` 冻结的 145,000
是从 LF 工作副本量的。于是：

- **任何新克隆都是红的**（工作区绿，克隆红）；
- Linux runner（LF）与 Windows runner（CRLF）对同一个文件判定**互相矛盾**；
- 这张表此前"看起来"已经在 4f927be 验证过了 —— 但它只在工作区验证过。

`test_canonical_package_discipline.py` 是反方向的同一种病：预算按 CRLF
**原样**冻结，Windows 上余量恰好为 0（加一行就红），Linux 上才几百字节。
两张表用了相反的两个口径，后人抄哪个都是错的。

### 28.2 修法：归一化，而不是重新冻结

重新冻结成 CRLF 只能解决一半 —— 大文件在 Linux（LF）下又会因为
"预算 - 尺寸 > 4,096" 撞上 `test_size_budgets_are_not_vacuous`。
**字节预算和混合换行符本质上不兼容**，所以两处闸门都改为测量归一化字节数
（`raw.replace(b"\r\n", b"\n")`），CRLF 与 LF 计为相同。

口径一变，三个原本"合规"的预算立刻暴露出真实的虚假宽松：

| 模块 | 旧预算 | 归一化后余量 | 新预算 |
| --- | --- | --- | --- |
| `report_core.py` | 95,000 | 5,296（>4,096） | 91,752 |
| `gui.py` | 98,000 | 6,030（>4,096） | 94,018 |
| `signal_lifecycle_core.py` | 56,000 | 4,703（>4,096） | 53,345 |

### 28.3 连带结论：`gui_core` 的"134 字节窒息"是换行符造出来的

此前多轮记录都把 `gui_core.py` 当成下一个窒息点（"只剩 134 B，约 3 行"）。
那是 CRLF 原始尺寸 104,866 对预算 105,000 的差。**归一化后是 102,352，
余量 2,648 字节** —— 大约 65 行，不是 3 行。

所以 gui_core 拆分**从"被预算倒逼"降回"普通优化"**，排在 T4（大模块补测试）
之后更合理。这也说明：**凡是拿字节数当判据的结论，都要先确认口径。**

### 28.4 新增的可复用做法

24. **干净树验证要用 `git clone`，不能用 `git archive`** —— 后者导出物里没有
    `.git`，依赖 `git show <sha>:<file>` 的等价性测试会全部误报 ERROR
    （这次误伤了 `test_signal_lifecycle_extraction_equivalence` 的 3 个用例）。
25. **任何"以字节为单位"的门禁，都必须显式处理换行符**，否则它衡量的
    是检出方式而不是代码。判据：同一份代码在 Linux 和 Windows 上跑出不同结果。
26. **口径修正会连带推翻已有结论** —— 这次一口气改了三个预算的合规性判断
    和一个模块的紧迫性排序。**换了更准的尺子，要去重测旧结论，不只是修当下的红。**

---

## 29. 整体重新分析（2026-09-15）：三处先前判断被证据推翻

产出：`REFACTOR_PLAN.md`。本轮为只读分析，未改生产代码。

### 29.1 被推翻 / 被降级的判断

| 先前判断 | 复核结果 | 证据 |
|---|---|---|
| 一字板只在 `auction_structure.py` 建模，回测路径缺失 | ❌ 推翻 | `tradeability.py:165-168` 返回 `locked_limit_up`；`tradeability_acceleration_v80.py:109-120` 有向量版；`analytics_core.py:133` 直接 import |
| `config` vs `config_core` 并行常量漂移是"可复现性"直接威胁 | ⚠️ 降级 | 89 个常量不同，但 `config_core` 真实消费方只有 1 个（`historical_backtest.py:43` 取 `OUTPUT_DIR`）。所有 core 模块都 `from config import` |
| 次新股过滤缺失 | ✅ 证实且更彻底 | 全仓唯一匹配是 `tests/golden_downloader_core.py:155` 的**假 ticker 名** `"N 新股"` |

**教训（第 8 例同类）**：上一轮的关键词扫描用「一字 / 一字板」中文词去搜，而回测路径用的是英文
`locked_limit_up`，于是得出"回测路径缺失"的错误结论。**跨语言代码库做关键词覆盖扫描，必须同时
搜中英文两套词表，否则覆盖率结论本身不可信。**

### 29.2 新确认的关键事实

- **overlay 层体积超过 core 层**：730,915 B vs 658,540 B（+11%）。补丁比本体还大。
- **A 股执行现实性建模成熟度高于预期**：涨跌停（`price_limit_policy.py` 覆盖北交所 30% /
  创业板 20% / 科创板 20% / 科创 ETF 20% / 主板 10%，标量+向量双实现）、一字板、T+1
  （`conditional_fill_v96`）、停牌、集合竞价、佣金印花税滑点，均已落地。
- **最低历史门槛 `300` 硬编码 9 处、`21` 硬编码 1 处**，散落在 7 个文件。改门槛需同步 9 处，
  漏一处即造成快路径与精确路径样本集静默不一致。
- **测试 : 生产 = 1 : 5.3**（11,325 / 59,530 行）；67/122 根模块被任何测试点名，
  55 个连名字都不出现。其中最危险的是 `conditional_fill_v96.py`（16,607 B，决定成交价）。
- `BACKTEST_PROVENANCE_VERSION` 在 `config_core` 停留在 `2026-08-09-v30`，落后约 30 个版本。

### 29.3 路线图要点

Phase 0（并行确定性证明，零生产改动）→ Phase 1（次新股 shadow 指标）→ Phase 2（300/21 提升为常量）
→ Phase 3（三个语义锁）→ Phase 4（web_report 三合一）→ Phase 5（死分叉 + 依赖降级）。

暂缓：打包 / `gui_core` 拆分 / 27 个 plain 模块归属 / 28 组重复体去重。

### 29.4 可复用经验（续）

27. **先证明可复现性，再动功能。** 本轮把「并行参数是否会改变回测数值」识别为全局解锁点：
    Phase 1~3 全部依赖 golden 等价性验证，若并行参数会改变数值，所有 golden 都不可靠。
    一个纯测试文件（Phase 0）决定后面所有阶段能否被安全验证。
28. **新能力一律先 shadow（只写不读）。** 次新股过滤会改变回测样本集 → 违反「回测结果可复现」
    不改项。改为先只加列不参与决策，用观测数据决定是否启用。这是「独立上线 + 风险可控」的通用解法。
29. **把魔法数字提升为常量时，取值必须保持不变。** 值不变 → 既有 golden 全绿本身就是等价性证明；
    反向验证时改一个值，必须至少有一个既有测试变红，才能证明常量真的被所有引用点消费。
