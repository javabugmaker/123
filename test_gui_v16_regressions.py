...
class GUIV16FilterRegressionTests(unittest.TestCase):
    ...
    def test_fundamental_filter_uses_actual_quality_fields(self):
        ...
        self.assertTrue(
            scanner._row_matches_filters(
                indexes,
                passed,
                "",
                filter_values=base + ("通过", "全部等级", "全部回测", "全部分数"),
            )
        )
        self.assertTrue(
            scanner._row_matches_filters(
                ...
                failed,
                "",
                filter_values=base + ("未通过", "全部等级", "全部回测", "全部分数"),
            )
        )
        self.assertTrue(
            scanner._row_matches_filters(
                ...
                missing,
                "",
                filter_values=base + ("数据缺失", "全部等级", "全部回测", "全部分数"),
            )
        )
        self.assertTrue(
            scanner._row_matches_filters(
                ...
                etf,
                "",
                filter_values=base + ("ETF不适用", "全部等级", "全部回测", "全部分数"),
            )
        )
    ...
    def test_asset_tier_backtest_and_score_filters_compose(self):
        ...
        self.assertTrue(
            scanner._row_matches_filters(indexes, row, "", filter_values=filters)
        )
        self.assertFalse(
            scanner._row_matches_filters(
                indexes, row, "", filter_values=filters[:-1] + ("≥40",)
            )
        )
        self.assertFalse(
            scanner._row_matches_filters(
                ...
                row,
                "",
                filter_values=filters[:6] + ("ETF",) + filters[7:],
            )
        )

    def test_legacy_quality_filter_remains_compatible(self):
