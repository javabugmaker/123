...
AKShare 无需 API Key，只负责低频财报，不参与 OHLCV、标的池或任何行情下载。程序通过
`stock_yjbb_em` 按报告期批量取得全市场业绩报表，并保存 `ReportPeriod` 与
`AnnouncementDate`：

- 同一天且股票池、目标报告期未变化：直接读取，不重复联网
- 新季度结束后：逐日检查是否出现新公告；未公告时保留上一份已知财报
- 法定披露窗口内标记为 `AWAITING_RELEASE`，过期仍无新报告才标记为 `STALE`
- 同一报告期只请求一次，不再对 5300 多只股票逐股访问
- 上游请求严格串行并带超时；批次落地后在本地向量化拆分股票
- 最新季度优先；完成后先发布中间缓存，再回填最近三份年报
- 结果与“已检查但无数据”状态写入 fsync 日志；GUI 取消后可以续跑
- 汇总阶段一次性分组、筛选和透视全市场财报，不再逐股反复扫描整张明细表
- 刷新失败：保留已有缓存，行情扫描继续运行
- GUI 勾选“刷新财报数据”：强制刷新

财报汇总包含最新报告期、公告日、ROE、毛利率、净利润同比、经营现金流/净利润估算及
最近三份年报净利润。历史回放只使用目标日期之前已经公告的记录，避免把后来发布的财报
提前用于信号。AKShare 批量业绩报表未提供的资产负债率等字段保持未知，不会伪造；原
“机构覆盖家数趋势”旧字段
只为读取历史结果保留，不再参与质量门槛、完整度或质量系数。超过法定披露期限仍未更新的
财报按数据未知处理，不会拿到完整财报质量系数。

AKShare 当前财报接口来自东方财富公开数据。Windows 下若启用 Clash 系统代理，程序会把
WinINET 代理同步到 Requests 环境；接口失败或超时时保留上一份有效缓存，不阻断行情扫描。
...
GUI 行情源固定显示为 `TickFlow Free`，财报来源为 `AKShare（公告日缓存）`。
...
- **财报层**：AKShare 只做低频公告日财报缓存，不参与日常行情扫描热路径；
  每个报告期只获取一次全市场截面，上游串行、本地向量化。最新季度与历史年报分阶段执行，
  写入可恢复日志，GUI 同步显示进度；本地汇总使用批量列运算。
