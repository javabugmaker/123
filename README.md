...
- 6–8 个独立进程各自维持 BaoStock 会话，避免共享全局 Socket
- 最新季度优先；完成后先发布中间缓存，再回填最近三份年报
- 每 5 只把结果与“已检查但无数据”状态写入 fsync 日志；GUI 取消会先落盘再退出，随后续跑
...
- **财报层**：BaoStock 只做低频点时财报缓存，不参与日常行情扫描热路径；
  按股票跨 4–8 个进程并行，每个进程拥有独立长连接。最新季度与历史年报分阶段执行，
  每 5 只写入可恢复日志，GUI 同步显示速度、成功率和 ETA；本地汇总使用批量列运算。
