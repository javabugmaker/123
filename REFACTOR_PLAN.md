...
**另试过、已放弃**：拿真实 `output/runs/*/AllResults.csv`（6835 行 × 447 列）
当回归夹具。结论是**不可行**，原因值得记下以免重复：

最近的 run 是 `20260915`，而 `DecisionPolicyParameterDigest` 由 `1ef2f9e`
（2026-09-16，§10 S6）引入——旧 run 的表头里根本没有这一列（已核实：
`grep -c` 为 0），签名内容也随 S6 的拆分变了。所以跑真实帧必然报两条：
`mismatched DecisionPolicySignature` 与 `missing DecisionPolicyParameterDigest`。

**产出物的 schema 随代码演化，旧 run 天然不合规**，拿它当门禁只会得到一个
永远红且无意义的测试。要验证就得用当次运行生成的内存帧，或重跑一次扫描。
