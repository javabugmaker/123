"""Network proxy helpers used only by the low-frequency AkShare fundamentals path."""
...
def _windows_system_proxy() -> dict[str, str]:
    """Read WinINET proxy used by Clash system-proxy mode on Windows."""
...
def configure_akshare_proxy_from_system() -> dict[str, str]:
    ...
    Shell-provided HTTP(S)_PROXY values take precedence. Values installed by
    this helper are removed again when the Windows system proxy is disabled.
    """
